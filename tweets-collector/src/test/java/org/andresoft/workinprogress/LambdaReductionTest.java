...
public class LambdaReductionTest
{
    @Test
    public void testLambdaReduction()
    {
        ...
        double average = roster.stream().filter(p -> p.getGender() == Person.Sex.MALE).mapToInt(Person::getAge).average()
                .getAsDouble();
        ...
        Integer totalAgeReduce = roster.stream().map(Person::getAge).reduce(0, (
                                                                                a,
                                                                                b) -> a + b);
